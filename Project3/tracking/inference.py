...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.allPositions:
            beliefCopy[pos] = self.getObservationProb(noisyDistance=observation,
                                                      pacmanPosition=currPos,
                                                      ghostPosition=pos,
                                                      jailPosition=jail) * self.beliefs[pos]
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        blankDist = DiscreteDistribution()

        for pos in self.allPositions:
            nextPosProb = self.getPositionDistribution(gameState, pos)
            for nextPos in nextPosProb.keys():
                blankDist[nextPos] += nextPosProb[nextPos] * self.beliefs[pos]

        self.beliefs = blankDist
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = self.legalPositions * quotient + self.legalPositions[:remainder]
    ...
    def observeUpdate(self, observation, gameState: busters.GameState):
        ...
        # Add all of the probabilities associated with each position to normalize later
        for part in self.particles:
            blankDist[part] += self.getObservationProb(observation, pacmanPos, part, jail)

        # If all probabilities are 0, reinitialize
        if blankDist.total() == 0:
            self.initializeUniformly(gameState)
        else:
            blankDist.normalize()
            for i in range(self.numParticles):
                # Reset particles to a sampling distribution of legal pos
                # Over time should have higher count of a legal pos wherever
                # the probability is highest
                self.particles[i] = blankDist.sample()
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Replace each particle with a random sampling of its distribution using self.getPositionDistribution
        # Autograder takes like 3 minutes to run Q5 so I'm going to use a dict here for nextProb lookups to save time
        nextProbs = {}
        for i in range(self.numParticles):
            part = self.particles[i]
            if part in nextProbs:
                self.particles[i] = nextProbs[part].sample()
            else:
                nextProbs[part] = self.getPositionDistribution(gameState, self.particles[i])
                self.particles[i] = nextProbs[part].sample()
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Particles should now be a tuple of all combinations of position of alive ghosts
        allGhostPos = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(allGhostPos)

        # Same method as standard ParticleFilter
        quotient = self.numParticles // len(allGhostPos)
        remainder = self.numParticles % len(allGhostPos)
        self.particles = allGhostPos * quotient + allGhostPos[:remainder]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # For all of the possible positions of the ghosts...
        for i in range(self.numParticles):
            # Establish the bayes net initial for the particle to be 1
            # Eventually, after enough multiplication, the weight will approach the true value
            nextProb = 1
            for j in range(self.numGhosts):
                # The probabilities for each ghost is multiplied since it's joint
                # self.particles[i][j] is needed for each of the ghost positions
                nextProb *= self.getObservationProb(observation[j], pacmanPos,
                                                    self.particles[i][j], self.getJailPosition(j))
            # Apply a probability to that combination of ghost positions
            blankDist[self.particles[i]] += nextProb

        if blankDist.total() == 0:
            self.initializeUniformly(gameState)
        else:
            blankDist.normalize()
            # Replace each particle with a random sample of the distribution for that combination
            for i in range(self.numParticles):
                self.particles[i] = blankDist.sample()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            # Simply just replace the ghost position with a sampling of the distribution
            # of that ghost position.
            for i in range(self.numGhosts):
