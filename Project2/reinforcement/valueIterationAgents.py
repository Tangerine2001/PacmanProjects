...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for i in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                ...
                for action in self.mdp.getPossibleActions(state):
                    computedQ = self.computeQValueFromValues(state, action)

                    # Picks argMAX of the the computedQValues for each move
                    if q == None or q < computedQ:
                        q = computedQ

                # if TERMINATING is reached, q will still be None, so default to 0
                valuesCopy[state] = q if q is not None else 0
    ...
    def computeQValueFromValues(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        # q will hold the sum of all the expected values of each successors state
        q = 0
    ...
    def computeActionFromValues(self, state):
        ...
        # This means the state is TERMINATING
        if len(actions) < 1:
            return None
        ...
        # Gets the largest q value from the surrounding area and sets the action to be that action
